...
const puppeteerConfigFileURL = new URL(
...
export function renderMermaidGraphsPlugin(): Plugin {
  const existingGraphFileNamesPromise = mkdir(graphsDirectory, {
    recursive: true,
  })
    ...
    if (!existingGraphFileNames.has(outFile)) {
      ...
      const { stdout, stderr } = await execPromise(
        `npx mmdc --configFile ${fileURLToPath(
          configFileURL
        )} --puppeteerConfigFile ${fileURLToPath(
          puppeteerConfigFileURL
        )} --input ${fileURLToPath(inFileURL)} --output ${fileURLToPath(
          outFileURL
        )}`
      );
        ...
        await Promise.all(
          mermaidCodeBlocks.map(async (codeBlock, index) => {
            ...
            if (!existingGraphsByName.has(outFile)) {
              existingGraphsByName.set(
                outFile,
                renderGraph(codeBlock, outFile, hash)
              );
            }
